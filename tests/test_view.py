...
def create_testing_complex_wavefield( nxny ):
    ...
    phi_phs = np.exp( 1j * 2 * np.pi * ( xv ** 2 / (2 * 20 * nx) + yv ** 2 / (2 * 3 * ny) ) )
...
def test_resize_complex_image():
    ...
    cv_interp = [ cv.INTER_NEAREST, cv.INTER_LINEAR, cv.INTER_CUBIC, cv.INTER_AREA, cv.INTER_LANCZOS4 ]
