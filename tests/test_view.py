...
def create_color_wheel( nxny, Hmin = -np.pi, Hmax = +np.pi ):
    ...
    H = np.angle( np.exp( 1j * (H - np.pi/2 )))  # global phase offset so that 0 and +\- pi are on the horizontal axis
    ...
    hsv_img[ ..., 0 ] = ( H + np.pi ) / ( np.pi * 2 )   # Rescale hue to the range [0, 1]
    hsv_img[ ..., 2 ] = V
...
def test_complexHSV_to_RGB_color_wheel_color_rectangle():
    ...
    np.testing.assert_array_almost_equal( color_rectangle2, color_rectangle, decimal = 3 )
    ...
    sxsy = ( 201, 101 ) 
    color_rectangle, phs, mag = create_color_rectangle( sxsy, -np.pi/3, +np.pi/2 )
...
if __name__ == '__main__':
    ...
    sz = ( 4, 7, 6 )
    phi = ( 2 * np.random.rand( *sz ).astype( np.float32 ) - 1 ) + 1j * ( 2 * np.random.rand( *sz ).astype( np.float32 ) - 1 )
