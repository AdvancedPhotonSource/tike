...
def test_variable_intensity():
    ...
    with (
        h5py.File(os.path.join(_dir, 'variable_intensity_input0.mat'), 'r') as input0,
        h5py.File(os.path.join(_dir, 'variable_intensity_input1.mat'), 'r') as input1,
        h5py.File(os.path.join(_dir, 'variable_intensity_output.mat'), 'r') as output,
    ):
    ...
    new_weights = cp.asnumpy(_get_coefs_intensity(
        cp.asarray(weights),
        cp.asarray(xi),
        cp.asarray(P),
        cp.asarray(O),
