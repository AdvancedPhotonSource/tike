...
class TestConvolution(unittest.TestCase, OperatorTests):
    ...
    def setUp(self):
        ...
        kernel = tike.random.numpy_complex(
            self.ntheta, self.nscan, self.nprobe, self.probe_shape, self.probe_shape
        )
    ...
    def test_adjoint_all(self):
        """Check that the adjoint operator is correct."""
        d = self.operator.fwd(
            **{self.m_name: self.m, self.m1_name: self.m1},
            **self.kwargs2,
        )
        assert d.shape == self.d.shape
        m, m1 = self.operator.adj_all(
            **{self.d_name: self.d, self.m_name: self.m, self.m1_name: self.m1},
            **self.kwargs2,
        )
