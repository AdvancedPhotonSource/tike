def eq2us(f, x, n, eps, xp):
    ...
    fe = xp.zeros([2*n, 2*n, 2*n], dtype=xp.complex64)
    ...
    Fe = xp.zeros([2*n+2*m, 2*n+2*m, 2*n+2*m], dtype=xp.complex64)
    Fe[idx+m, idy+m, idz+m] = Fe0[idx0, idy0, idz0]
    Fe = Fe.flatten()

    # smearing operation (F=Fe*kera), gathering
    F = xp.zeros(x.shape[0], dtype=xp.complex64)
    ...
    for i0 in range(2*m):
        for i1 in range(2*m):
            for i2 in range(2*m):
                kera = xp.sqrt(xp.pi)**3/xp.sqrt(mu*mu*mu)*(xp.exp(-xp.pi**2/mu*((ell0-m+i0)/(2*n)-x[:, 0])**2
                                                                   - xp.pi**2/mu *
                                                                   ((ell1-m+i1) /
                                                                    (2*n)-x[:, 1])**2
                                                                   - xp.pi**2/mu*((ell2-m+i2)/(2*n)-x[:, 2])**2))
...
def us2eq(f, x, n, eps, xp):
    ...
    # smearing kernel (ker)
    ker = xp.zeros((2*n, 2*n, 2*n), dtype=xp.complex64)
    [xeq0, xeq1, xeq2] = xp.mgrid[-n//2:n//2, -n//2:n//2, -n//2:n//2]
    ker[n//2:n//2+n, n//2:n//2+n, n//2:n//2 +
        n] = xp.exp(-mu*xeq0**2-mu*xeq1**2-mu*xeq2**2)

    # smearing operation (G=f*kera)
    G = xp.zeros([(2*n+2*m)*(2*n+2*m)*(2*n+2*m)], dtype=xp.complex64)
